...
import htmllib, formatter, urllib, urlparse, HTMLParser
#htmlentitydefs
...
class Release:
    ...
    def __init__(self, id: str, name: str='?', group: str='?', type: str='?', date: str='?', dls : list[str] =[]):
        self.id = int(id)
        self.name = name
        self.group = group
        self.date = date
    ...
    def update(self):
        ...
        m = re.compile(r'.*(\d\d\d\d)').match(self.date)
        if m:
            self.year = m.group(1)
        else:
            self.year = '????'
    ...
    def download_from_url(self, url: str, targetdir: str, to_d64: bool=False, to_prg: bool =False):
