...
def test_tosec_multiple_flags():
    ...
    assert result.flags == "[a h m]"
    # The specific score values are not tested since score is only used for ordering
...
def test_tosec_complex_filename():
    ...
    filename = Path(
        "Cannon Fodder (1993)(Virgin Interactive)(A1200)(PAL)(GB)(en)(Disk 1 of 2)[h].adf"
    )
    result = tosec.from_file(filename)
    ...
    assert result.flags == "[h]"  # 'h' flag present
